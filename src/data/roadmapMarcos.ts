...
import {
  Monitor, Palette, MessageSquareQuote, Search, Layers, Package,
  HeartHandshake, Accessibility, MessagesSquare, Smartphone, Handshake, Gift,
} from "lucide-react";
 ...
 *
 * A ordem na onda vem do campo `data`; marcos sem data confirmada usam
 * `dataAssumida`/`semData` conforme o caso (ver comentário de cada item).
 */
...
export interface Marco {
  ...
  href?: string;
  /** Data estimada — não há registro exato da data de entrega. */
...
export const marcos: Marco[] = [
  ...
  },
  {
    ...
    continuo: true,
    descricao:
      "Publicação, seção por seção, do guia com a visão geral de cada produto do ecossistema.",
    detalhes: [
  ...
  },
  {
    ...
    tone: "brick",
    descricao:
      "Portfólio com os kits, brindes e materiais impressos desenvolvidos pelo time, organizado por categoria.",
    detalhes: [
  ...
  },
  {
    ...
    // Sem data exata registrada — estimada para o início de julho, logo
    // depois do catálogo de produtos físicos.
  ...
  {
    // Sem data confirmada — a integração foi adiada. A âncora só posiciona
    // o marco na trilha; a interface não mostra data nenhuma.
    id: "integracao-chat",
    icon: Handshake,
