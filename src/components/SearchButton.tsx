...
export function SearchButton() {
  ...
  return (
    <button
      ...
      className="group flex items-center gap-2 h-9 rounded-lg bg-background text-muted-foreground hover:text-foreground hover:bg-muted transition-colors px-2.5 sm:px-3"
    >
      <Search className="h-4 w-4" />
      <span className="hidden lg:inline text-xs font-roboto">Buscar…</span>
      <kbd className="hidden lg:inline-flex items-center gap-0.5 rounded bg-muted px-1.5 py-0.5 text-[10px] font-roboto font-semibold text-muted-foreground">
        {isMac ? "⌘" : "Ctrl"} K
      </kbd>
    </button>
  );
